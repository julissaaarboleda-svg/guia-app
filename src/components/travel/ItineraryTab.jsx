...
For other items (hotels, restaurants, tours, events): return one item per booking. Set date (yyyy-MM-dd — for a hotel stay output separate check-in and check-out items using their respective dates; empty string if none), time (HH:MM 24h or empty), activity (short label e.g. "Hotel check-in", "Hotel check-out", "Dinner reservation"), name (the venue, hotel, or restaurant name if stated, otherwise empty string), location (the CITY name e.g. "Rio de Janeiro" — never the hotel or venue name, never an airport code), address (full street address if listed, otherwise empty string), link (the venue/hotel/restaurant website URL if stated, otherwise empty string), notes (confirmation number and key booking details).

Only include real travel/booking items. Return as { activities: [...] }.`,
        file_urls: [file_url],
